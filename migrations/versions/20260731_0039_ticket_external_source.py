"""Add external_source/external_id to Ticket, mirroring the columns
ConfigurationItem already has (netbox_sync.py, cmdb_import.py). Used by
serviceops_core/rt_import.py so a re-run of the Request Tracker import
matches already-imported tickets by RT ticket id instead of duplicating
them.

Revision ID: 20260731_0039
Revises: 20260731_0038
"""
from alembic import op
import sqlalchemy as sa

revision = "20260731_0039"
down_revision = "20260731_0038"
branch_labels = None
depends_on = None


def upgrade():
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    if "ticket" not in inspector.get_table_names():
        return
    columns = {c["name"] for c in inspector.get_columns("ticket")}
    with op.batch_alter_table("ticket") as batch_op:
        if "external_source" not in columns:
            batch_op.add_column(sa.Column("external_source", sa.String(length=20), nullable=True))
        if "external_id" not in columns:
            batch_op.add_column(sa.Column("external_id", sa.String(length=120), nullable=True))


def downgrade():
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    if "ticket" not in inspector.get_table_names():
        return
    columns = {c["name"] for c in inspector.get_columns("ticket")}
    with op.batch_alter_table("ticket") as batch_op:
        if "external_id" in columns:
            batch_op.drop_column("external_id")
        if "external_source" in columns:
            batch_op.drop_column("external_source")
