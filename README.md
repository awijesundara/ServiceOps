...
[![Version](https://img.shields.io/badge/version-1.78.3-003E4C)](VERSION)
...
The following procedure installs the current stable release, **v1.78.3**, on a
...
curl -fLO https://github.com/awijesundara/ServiceOps/releases/download/v1.78.3/serviceops-1.78.3-1.el9.noarch.rpm
curl -fLO https://github.com/awijesundara/ServiceOps/releases/download/v1.78.3/serviceops-1.78.3-1.el9.noarch.rpm.sha256

sha256sum -c serviceops-1.78.3-1.el9.noarch.rpm.sha256
```

Do not continue unless checksum verification reports `OK`. Packages for the
other supported platforms are available on the
[v1.78.3 release page](https://github.com/awijesundara/ServiceOps/releases/tag/v1.78.3).

### 3. Install and initialize ServiceOps

```bash
sudo dnf install -y ./serviceops-1.78.3-1.el9.noarch.rpm
...
Health should report status `ok` and version `1.78.3`.
