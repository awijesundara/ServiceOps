...
def test_attachment_alongside_an_inline_text_body_is_still_imported(app, monkeypatch):
    ...
    with a screenshot) -- the original code returned early as soon as it
    saw inline Content, so it never even looked for attachments on the
    (common) case where a transaction has both."""
    with app.app_context():
        admin_id = User.query.filter_by(username="admin").one().id
        pdf_bytes = b"%PDF-1.4 fake pdf content"
        ticket = make_ticket(903, "Ticket with body and attachment together", requestor_ids=["9"])
        factory = enable_rt(
            monkeypatch, tickets=[ticket], queues={"1": "General"},
            users={"9": {"EmailAddress": "leo@example.test", "RealName": "Leo"}},
            history={"903": [{"id": "77"}]},
            transactions={"77": {"Type": "Correspond", "Content": "Please see attached report", "Creator": {"id": "9"}}},
            transaction_attachments={"77": [{"id": "603"}]},
            attachment_meta={"603": {
                "Filename": "report.pdf",
                "ContentType": "application/pdf",
                "Content": base64.b64encode(pdf_bytes).decode(),
        ...
        result = import_from_rt(1, actor_user_id=admin_id, session_factory=factory)
        assert result["attachments_imported"] == 1
        record = EnterpriseRecord.query.filter_by(external_source="rt", external_id="903").one()
        assert "Please see attached report" in record.description
        assert "report.pdf" in record.description
        assert FileAttachment.query.filter_by(enterprise_record_id=record.id).count() == 1


def test_unresolvable_requestor_uses_placeholder_not_the_importing_admin(app, monkeypatch):
    """A RT Requestor with no usable email (deleted RT user, autogenerated
    mailbox, etc.) must not be credited to whoever happens to run the
    import -- that misattributes the ticket to a real admin account. It
    should land on a clearly-labeled placeholder account instead."""
    with app.app_context():
        admin_id = User.query.filter_by(username="admin").one().id
        ticket = make_ticket(904, "No resolvable requestor", requestor_ids=["9"])
        factory = enable_rt(
            monkeypatch, tickets=[ticket], queues={"1": "General"},
            users={"9": {"EmailAddress": "", "RealName": "Ghost User"}},
        )
        import_from_rt(1, actor_user_id=admin_id, session_factory=factory)
        record = EnterpriseRecord.query.filter_by(external_source="rt", external_id="904").one()
        assert record.requester.email == "unresolved-rt-requester@rt-import.invalid"
        assert record.requester_id != admin_id
